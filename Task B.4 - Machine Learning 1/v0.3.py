...
def load_and_process_data(ticker, start_date, end_date, features=['Open', 'High', 'Low', 'Close', 'Volume'],
    ...
    data_directory = os.path.expanduser(
        '~/Desktop/COS30018 - Intelligent Systems/Assignment_B/Task B.4 - Machine Learning 1')
...
def create_dl_model(input_shape, num_layers, units_per_layer, layer_type, dropout_rate=0.2, use_bidirectional=False,
    ...
    # Optionally apply activation function after the first layer (if provided).
    if activation:
        model.add(Activation(activation))

    # Optionally apply batch normalistion to standardise inputs to the next layer.
    if use_batchnorm:
